...
const DIVIDENDS = [
  { ticker: "PETR4", name: "Petrobras", type: "Dividendo", value: "0,52", dateCom: "25/04/2026" },
  { ticker: "VALE3", name: "Vale", type: "JCP", value: "2,73", dateCom: "11/04/2026" },
  { ticker: "ITUB4", name: "Itaú", type: "Dividendo", value: "0,017", dateCom: "30/04/2026" },
  { ticker: "BBDC4", name: "Bradesco", type: "JCP", value: "0,017", dateCom: "01/05/2026" },
  { ticker: "BBAS3", name: "Banco do Brasil", type: "Dividendo", value: "0,41", dateCom: "13/05/2026" },
];

const ARTICLES = [
  { id:"cdb-vs-tesouro", cat:"Investimentos", title:"CDB vs Tesouro Direto", desc:"Qual rende mais para sua reserva?", conteudo: "<h3>CDB vs Tesouro</h3><p>Ambos são excelentes para reserva de emergência...</p>" },
  { id:"sair-cheque-especial", cat:"Crédito", title:"Sair do Cheque Especial", desc:"Como fugir dos juros de 150% ao ano.", conteudo: "<h3>Cheque Especial</h3><p>Fuja dos juros abusivos...</p>" },
];
...
export default function App() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const resAi = await fetch("/api/groq", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ messages: [{ role: "system", content: "Analista financeiro. Resumo curto do mercado e o que fazer hoje." }, { role: "user", content: `Dados: ${Object.entries(dataM).map(([k,v]) => `${v.name}: ${v.bid}`).join(", ")}` }]}) });
            ...
            {tab === "dividendos" && (
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>{DIVIDENDS.map(d=>(<tr key={d.ticker}><td><span className="ticker">{d.ticker}</span><span className="name-sub">{d.name}</span></td><td>{d.type}</td><td>{fmtBRL(parseFloat(d.value.replace(",",".")))}</td><td>{d.dateCom}</td></tr>))}</tbody>
                </table>
              </div>
            )}
          </>
